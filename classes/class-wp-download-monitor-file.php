...
*/
class WP_Download_Monitor_File {
	...
	/**
	 * Loads all download data from custom fields
	 *
	 * @param   int		$id		ID of the download to load
	 */
	function __construct( $id ) {
		...
		foreach ( $load_data as $key => $default ) 
			$this->$key = (isset($this->custom_fields['_' . $key][0]) && $this->custom_fields['_' . $key][0]!=='') ? $this->custom_fields['_' . $key][0] : $default;
	}

    /**
     * get_download_count function.
     *
     * Gets the total hits for all versions
     * 
     * @access public
     * @return int     
     */
    function get_download_count() {
    	...
    	foreach ( $this->get_children() as $child ) {
    		$count += get_post_meta( $child->ID, 'hits', true );
    	}
    	return $count;
    }
     ...
     * 
     * @access public
     * @param string $size (default: 'download_monitor_thumbnail')
     */
    function get_image( $size = 'download_monitor_thumbnail' ) {
    	global $download_monitor;
    	
    	if ( has_post_thumbnail( $this->id ) )
			echo get_the_post_thumbnail( $this->id, $size ); 
		else
			echo '<img src="'. apply_filters('download_monitor_placeholder_img_src', $download_monitor->plugin_url() . '/assets/images/placeholder.png' ) . '" alt="Placeholder" />'; 
    }
    
    /**
     * get_file function.
     * 
     * Get and return the main file for this download
     *
     * @access public
     * @return object
     */
	...
	function get_children() {
		...
		if ( ! is_array( $this->children ) ) :
        	...
        	if ( false === ( $this->children = get_transient( $transient_name ) ) ) :
		        ...
		        $this->children = get_posts( 'post_parent=' . $this->id . '&post_type=download_file&orderby=menu_order&order=ASC&fields=ids&post_status=any&numberposts=-1' );
